...
function getHistoryData(profile, logs) {
  if (!profile) return [];
  const map = new Map();
  ...
  return Array.from(map.entries()).map(([date, total]) => ({ date, total, goal: profile.dailyGoal }));
}

// ===================== USER PROFILE =====================
class UserProfile {
  constructor() { this.profile = null; this.loadProfile(); }
  ...
  hasProfile()   { return this.profile !== null && this.profile !== undefined; }
          ...
          <div class="form-grid-2">
            ...
            <div class="form-inline">
              ...
              <select class="form-select" name="weightUnit">
                <option value="kg"  ${(p.weightUnit || 'kg') === 'kg'  ? 'selected' : ''}>kg</option>
          ...
          <div class="form-grid-2">
            <div>
              ...
              <select class="form-select" name="activityLevel">
                ...
                <option value="Light" ${(p.activityLevel || 'Light') === 'Light' ? 'selected' : ''}>Lightly Active</option>
...
function bindSetupEvents(onSubmit) {
  ...
  checkbox?.addEventListener('change', () => medIcon?.classList.toggle('active', checkbox.checked));
...
function calculateHydrationGoal(profile) {
  ...
  const dailyWorkout = ((profile.exerciseMinutesPerSession || 0) * (profile.exerciseDaysPerWeek || 0)) / 7;
  total += dailyWorkout * 16.6;
  total += (profile.caffeineCups || 0) * 100;
  total += (profile.sodaCans     || 0) * 150;
  return Math.max(1200, Math.round(total));
}

// ===================== TODAY PAGE =====================
function renderToday(todayTotal, progressPct, todayLogs, streakStats, profile) {
  const fillLevel  = 100 - Math.min(100, Math.max(0, progressPct));
  const percentage = Math.min((todayTotal / profile.dailyGoal) * 100, 100);

  const presets = INTAKE_PRESETS.map((preset, idx) => `
    <button class="preset-btn" id="preset-${idx}" data-preset="${idx}">
      ...
      <div class="log-item">
        <div class="log-item-left">
          ...
          <div>
            ...
            <div class="log-meta">${new Date(log.timestamp).toLocaleTimeString([], { hour:'2-digit', minute:'2-digit' })} &bull; ${log.type}</div>
          </div>
        </div>
        <button class="btn-delete" id="del-${log.id}" data-delete="${log.id}">${icons.trash}</button>
      </div>`).join('');
      ...
      <form class="manual-form" id="manual-form">
        ...
        <div class="manual-field">
          ...
          <div class="manual-input-row">
            <input class="manual-input" type="number" id="manual-amount" placeholder="0" min="1">
...
function bindTodayEvents(onPresetClick, onManualSubmit, onDelete) {
  ...
  document.querySelectorAll('[data-delete]').forEach(btn => {
    btn.addEventListener('click', () => { if (onDelete) onDelete(btn.dataset.delete); });
  });
}
...
class AquaTrackerApp {
  ...
  addLog(amount, type = 'Water') {
    this.logs.unshift({ id: Math.random().toString(36).substr(2, 9), amount, type, timestamp: Date.now() });
    this.saveLogs();
    this.render();
  }
    ...
    app.innerHTML = `<div class="app">
      <header class="header">
        ...
        <div class="header-right">
          ${streakStats.currentStreak > 0 ? `<div class="streak-sm">${icons.flame}<span class="streak-count">${streakStats.currentStreak}</span><span style="font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.1em">Days</span></div>` : ''}
          <button class="btn-icon" id="btn-settings">${icons.settings}</button>
        </div>
      </header>
      <main class="main">
        ${this.activeTab === 'today'
          ? renderToday(todayTotal, progressPct, todayLogs, streakStats, profile)
          : renderTrends(streakStats, historyData)}
      </main>
      <nav class="bottom-nav">
        <div class="nav-inner">
          <button class="nav-btn ${this.activeTab === 'today'  ? 'active' : ''}" id="nav-today"  data-tab="today">
            ${icons.droplets.replace('width="20"','width="22"').replace('height="20"','height="22"')}
            <span>Today</span>
          </button>
          <button class="nav-btn ${this.activeTab === 'trends' ? 'active' : ''}" id="nav-trends" data-tab="trends">
            ${icons.history}
            <span>Progress</span>
          </button>
        </div>
